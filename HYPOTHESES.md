...
| **Deciding number** | **3.31 bps** gross capture at the 900 s horizon against **80.00 bps** round-trip maker cost. Coinbase BTC-USD. AUC **0.596** — the signal is real and the arithmetic still fails, by 23×. Net EV **−76.69 bps**. |
...
| **Limitations** | Days, not regimes. Under the project's own research-honesty rule this is pipeline validation, not evidence of a durable edge — a signal fitted on a few days is fitted on those days' volatility regime. |
...
| **What would reopen it** | **Not a better model and not lower fees.** The cost problem was genuinely solved here — 5.33 bps against crypto spot's 80 — and the signal vanished anyway: 3.31 bps of capture became 0.22, and AUC fell from 0.596 to coin-flip. The one figure clearing rounding noise (+0.215 bps at 900 s) **reversed to −0.785 bps** on a stride-1 control, which also cleared the bar-width confound. Reopens only on evidence of measurable AUC on a *different* instrument or feature class — never on a re-run of the same pipeline against the same contract with a stronger learner. |
